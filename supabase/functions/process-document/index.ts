...
serve(async (req) => {
  ...
  try {
    ...
    return json({ error: 'Vision OCR failed', details: String(e) }, 502)
  }
  ...
  if (docErr) return json({ error: 'DB write failed', details: docErr.message }, 500)

  return json({ document_id: doc.id, kind: doc.kind, ocr_data: doc.ocr_data })
})
